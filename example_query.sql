...
section_texts.parent_text, MATCH(section_texts.parent_text) against('Tableau*' in boolean mode) as relevance_parent_section,
(MATCH(o.text) against('"Portrait de femme' in boolean mode) +
MATCH(section_texts.child_text) against('Couture' in boolean mode) +
MATCH(section_texts.parent_text) against('Tableau*' in boolean mode)) as total_relevance
from object o
left join (
...
and
MATCH(section_texts.parent_text) against('Tableau*' in boolean mode))
